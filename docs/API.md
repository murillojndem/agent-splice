...
The runtime decides whether a response is a stream. That decision follows the RFC 9110 media-type grammar — the type is matched case-insensitively and the parameters are parsed, so `text/event-stream; charset=utf-8` is an event stream and `text/event-stream; ===` is not a media type at all. A `stream: true` request answered with an ordinary JSON body — an error, or a runtime that ignores the flag — is relayed unchanged with the runtime's own content type, and the exchange records that it was not streamed.
